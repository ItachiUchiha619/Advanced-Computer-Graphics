...
HeightMap::HeightMap(const  std::string& name) {
	...
	heightmapSize.y = vertexScale.y * 250.0f;//each  height  is a byte!
	...
	for (int i = 0; i < 10000; i++)
	{
		...
		while (isAdded == false)
		{
			randtemp = (rand() + (5*rand()/6)) % numVertices;

			if (pos1.empty() && posCheck.empty())
			{
				pos1.emplace_back(randtemp);
				posCheck.emplace_back(pos1[i]);
				isAdded = true;
			}
			else if (!pos1.empty() && !posCheck.empty())
			{
				int samePos = 0;
				pos1.emplace_back(randtemp);

				for (int j = 0; j < posCheck.size(); j++)
				{
					if (pos1[i] == posCheck[j])
					{
						samePos++;
					}
				}
	...
	{
		randPositions.emplace_back(vertices[pos1[i]]);
	}
}